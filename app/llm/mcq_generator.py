...
def _adaptation_reason_from_payload(adaptation_payload: dict, section_number: int) -> str:
    ...
    has_section_weak_topic = any(
        isinstance(weak_topic, dict) and weak_topic.get("section_number") == section_number
        for weak_topic in weak_topics
    )

    if mode == "adaptive" and has_section_weak_topic:
        return f"Adaptive question generated for section {section_number} because prior session history marks this section as weak."
    if mode == "adaptive":
        return "Targeted adaptive session run executed driven by historical knowledge base performance matrix parameters."
    return "Cold-start coverage question generated from the selected section because no prior relevant learning history exists."
            ...
            questions.append(
                {
                    ...
                    "topic": topic,
                    "difficulty": "medium",
                    "question": f"Section {section_number} question {question_number}: Which option best reflects {topic}?",
                    "options": {
                        "A": source_preview,
                    ...
                    "correct_answer": "A",
                    "explanation": "Option A is correct because it is directly grounded in the retrieved chunk from the selected section.",
                    "adaptation_reason": _adaptation_reason_from_payload(adaptation_payload, section_number),
...
def generate_mcqs(
    ...
    for i in range(0, len(selected_section_numbers), segment_size):
        ...
        target_chunks = [c for c in retrieved_chunks if c["section_number"] in target_sections]
        ...
        try:
            ...
            compiled_questions.extend(q.model_dump() for q in fallback_set.questions)

    return MCQSet.model_validate({"questions": compiled_questions})
    ...
    for attempt in range(MAX_LLM_GENERATION_ATTEMPTS):
        ...
        if attempt > 0 and last_error is not None:
            candidate_prompt = (
                ...
                "Every question text must be completely unique. Return valid JSON only."
            )
        ...
        try:
            ...
            for question in batch_mcq_set.questions:
                if not question.adaptation_reason or "without a section-specific" in str(question.adaptation_reason):
                    question.adaptation_reason = _adaptation_reason_from_payload(
                        adaptation_payload=adaptation_payload,
                        section_number=question.section_number
                    )
            return batch_mcq_set

        except (JSONDecodeError, ValidationError, ValueError) as error:
            last_error = error

    raise RuntimeError(f"LLM MCQ batch generation failed validation for sections {selected_section_numbers}: {last_error}")